...
for P in "${PROBABILITIES[@]}"; do
    ...
    ldpc_Utils/bbchannel -s $SEED -p $P $TEST_DATA $INPUT_FILE
    ...
    for ITER in "${MAX_ITERS[@]}"; do
        ...
        ERROR_RATE=$(ldpc_Utils/diffblock -n 1600 -m 8 $TEST_DATA $OUTPUT_FILE | grep "Error rates" | awk '{print $8}')
